...
theme.Discount = (function () {
  ...
  Discount.prototype = {
    init: function (variant) {
      ...
      var discountNumber = this._discountCalculates(
        decimalNumber,
        this.percentObject.discount_percent,
        (discount) => {
          return discount;
        }
      );
    ...
    */
      var currencySplit = this._currencySplit(currencyFormat, (parts) => {
        return parts;
      });

      this._currencyRender(currencySplit, this.selector, this.css);
    },
    /*
      ...
      _getVariant
    */
    _getVariant: function (variant, callback) {
      //console.log("Get price value named product variant, number comes as integer.",variant);
      return callback(variant);
    },
    _toDecimal: function (variant, callback) {
      var decimal = variant / 100;
      //console.log("Gets the product variant value to convert to a decimal number.",decimal);
      return callback(decimal);
    },
    _discountCalculates: function (decimal, percent, callback) {
      var discount = (decimal - (percent / 100) * decimal).toFixed(2);
      //console.log("Gets the already converted decimal number of the product variant and uses it to get the discount value by applying the percentage number.",discount);
     ...
     *
     */
    _currencyFormat: function (number, settings, callback) {
      var { lang, currency } = settings;
      var format = new Intl.NumberFormat(lang, {
        style: "currency",
        currency: currency,
      }).format(number);
      //console.log("Apply currency (pt-BR, BRL) symbol and convert number of discount variable to currency format",format);
    ...
    _currencyRender: function (parts, seletor, style) {
      var { symbol, whole, decimal } = parts;
      var target = $(seletor.discountPrice).html(`<span></span>`);
      var money = target.find("span");
      var amount = money
        .addClass(seletor.discountPriceClass)
        .html(`<sup>${symbol}</sup>${whole}<sup>${decimal}</sup>`);
...
slate.a11y = {
  ...
  /**
   * If there's a hash in the url, focus the appropriate elementF
   */
  focusHash: function () {
